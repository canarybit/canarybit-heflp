...
def get_alpha_gaus(values:ArrayLike, values_size:int=0, num_bits:int=16)->float:
    '''
    ...
    if values_size == 0:
        sigma:float = ((np.max(values) - np.min(values)) * gaussian_const) / ((2 * np.log(len(values))) ** 0.5)
    else:
        sigma:float = ((np.max(values) - np.min(values)) * gaussian_const) / ((2 * np.log(values_size)) ** 0.5)
